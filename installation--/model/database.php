...
class InstallationModelDatabase extends JModelBase
{
	...
	public function installSampleData($options)
	{
		...
		// Get the options as a object for easier handling.
		$options = JArrayHelper::toObject($options);
		$options->sample_file = "sample_data.sql";
	...
	protected function _splitQueries($query)
	{
		...
		for ($i = 0; $i < strlen($query) - 1; $i++)
		{
			...
			if ($in_string && $query[$i] == $in_string &&
				((!$db_esc_quote && strlen($query)-1>$i+1 && $in_string == $query[$i+1]) || ($db_esc_quote && $db_esc_quote < 2 && $buffer[1] == $in_string))){
				$db_esc_quote++;
			}
			else{
				$db_esc_quote = 0;
			...
			if ($in_string && ($query[$i] == $in_string) && $buffer[1] != "\\" && !$db_esc_quote)
			{
				$in_string = false;
			}
