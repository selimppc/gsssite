...
class InstallationModelConfiguration extends JModelBase
{
	...
	public function setup($options)
	{
        ...
        // force update of manifest caches
        @$this->refreshManifestCaches($options);
	...
	private function refreshManifestCaches($options)
	{
		// Get a database object.
		try
		{
		...
		$db->setQuery('SELECT extension_id ' . ' FROM ' . $db->quoteName('#__extensions'));
		$db->query();
		$eids = $db->loadColumn();
		...
		foreach ($eids as $id) {
			$result |= $this->doManifestRefresh((int)$id, $installer, $db);
		}
		return $result;
	}

	private function doManifestRefresh($id, &$installer, &$db)
	{
		if ($id) {
			$installer->extension = JTable::getInstance('extension', 'JTable', array('dbo' => $db));

			if (!$installer->extension->load($id)) {
				$installer->abort(JText::_('JLIB_INSTALLER_ABORT_LOAD_DETAILS'));
				return false;
			}

			$installer->loadAllAdapters();
			if (!$installer->setAdapter($installer->extension->type)) {
				return false;
			}

			if (is_object($installer->getAdapter($installer->extension->type))) {
				if (method_exists($installer->getAdapter($installer->extension->type), 'refreshManifestCache')) {
					$result = $installer->getAdapter($installer->extension->type)->refreshManifestCache();
					if ($result !== false) {
						return true;
					} else {
						return false;
					...
					$installer->abort(JText::sprintf('JLIB_INSTALLER_ABORT_METHODNOTSUPPORTED_TYPE', $installer->extension->type));
					return false;
				}
			}
			return false;
