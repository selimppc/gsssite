<?php
/**
* @version   $Id: layout.php 24732 2014-12-16 12:48:18Z arifin $
...
class GantryDropdownLayout extends AbstractRokMenuLayout
{
    ...
    protected function renderItem(JoomlaRokMenuNode &$item, RokMenuNodeTree &$menu)
    {
        ...
        $dd_large_direction = $item_params->get('dropdown_large_direction','rt-dropdown-right-large');
        $dd_desktop_direction = $item_params->get('dropdown_desktop_direction','rt-dropdown-right-desktop');
        $dd_tablet_direction = $item_params->get('dropdown_tablet_direction','rt-dropdown-right-tablet');
       ...
       if ($child_type!="menuitems") {
        ...
        ?>
        <li <?php if($item->hasListItemClasses()) : ?>class="<?php echo $item->getListItemClasses()?> <?php if ($item->hasChildren()) echo $dd_large_direction.' '.$dd_desktop_direction.' '.$dd_tablet_direction; ?>"<?php endif;?> <?php if($item->hasCssId() && $this->activeid):?>id="<?php echo $item->getCssId();?>"<?php endif;?>>
    ...
    public function renderMenu(&$menu) {
        ob_start();
?>
<div class="gf-menu-device-container responsive-type-<?php echo $this->args['responsive-menu'];?>">
